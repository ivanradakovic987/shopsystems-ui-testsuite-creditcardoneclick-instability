    ...
    iValidateSuccess
{
    ...
    public function fillPaymentMethodFields($paymentMethod, $paymentAction)
    {
        $actingPaymentMethod = $this->getActingPaymentMethod($paymentMethod);
        ...
        foreach ($paymentMethodConfig as $name => $value) {
            ...
            if (array_key_exists($name . '_text', $this->getLocator()->$paymentMethodPageLocator)) {
                $loc = $name . '_text';
                $this->preparedFillField($this->getLocator()->$paymentMethodPageLocator->$loc, $value);
            } elseif (array_key_exists($name.'_select', $this->getLocator()->$paymentMethodPageLocator)) {
                $loc = $name . '_select';
                $this->preparedSelectOption($this->getLocator()->$paymentMethodPageLocator->$loc, $value);
                ...
                $this->preparedCheckOption($this->getLocator()->$paymentMethodPageLocator->$loc);
            }
        }
